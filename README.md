...
| --- | --- | --- |
| 2026-07-10 | `v0.7.10-alpha.1654` | Agent Skills(agentskills.io)兼容性加固,跨客户端技能发现与解析更容错;修复 Intel 机型因组件超解压上限被拦致启动失败(上限提至 256MiB)。 |
| 2026-07-09 | `v0.7.9-alpha.1754` | 会话附加范围(scope)徽标改为对比度更高的靛蓝配色,文字清晰可见。 |
| 2026-07-09 | `v0.7.9-alpha.1323` | 「未找到 zWorkspace / zAgent 二进制」时上浮真实根因并附平台排查建议;附加资料本地文件措辞由「上传」改为「已就绪」。 |
| 2026-07-09 | `v0.7.9-alpha.0958` | SBOM 供应链审计覆盖与精度增强:递归子目录、新增 Deno / Julia 及 NuGet / yarn Berry / pnpm v5 识别,按生态逐一回退;修复 Linux 切换主题白屏冻死。 |
| 2026-07-08 | `v0.7.8-alpha.1535` | 修复用户多行输入渲染丢失换行:保留输入中的换行与空行,消息按原样多行显示。 |
| 2026-07-08 | `v0.7.8-alpha.0935` | 上下文占用圆环分档变色(75% 黄 / 90% 红),轨迹时间线步骤可点击跳转;修复工具网关(zMCP)后端缺席时误报「unknown tool」。 |
