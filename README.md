...
| --- | --- | --- |
| 2026-07-13 | `v0.7.13-alpha.2046` | 插件与模型提供方补充官方品牌图标、新增来源可信度盾牌,强化插件 / 技能签名验证与信任库(可导入根证书 PEM);修复模型提供方配置及消息元信息丢失。 |
| 2026-07-12 | `v0.7.12-alpha.1858` | 修复长时工具调用偶发中断(SSE EOF,解除 30 秒写超时截断),SBOM 离线漏洞库改用单文件索引提速,收紧插件签名验签门禁。 |
| 2026-07-12 | `v0.7.12-alpha.1523` | 消息栏 `/ws <名称>` 接入真实的工作区切换并持久保存,名称未匹配时返回可选工作区列表。 |
| 2026-07-12 | `v0.7.12-alpha.1400` | 新增 `/fork` 命令:复制当前会话为副本并在副本中继续,原会话不变;消息栏 `/stop`、`/resume` 接入真实暂停 / 终止。 |
| 2026-07-11 | `v0.7.11-alpha.1600` | 内置 AI 引导式技能创建器(`skill-creator`),新增市场发布契约兼容性检查工具(`market.check`);修复消息柄弹出菜单靠近屏幕底部被裁切。 |
