...
| --- | --- | --- |
| 2026-07-22 | `v0.7.22-alpha.1620` | 修复外接 MCP 工具调用被后端拒绝(进程身份改由带外通道传递);移除 MCP 服务后清理国密凭证库中的残留孤儿凭证。 |
| 2026-07-22 | `v0.7.22-alpha.1451` | 修复登录令牌轮换后添加外接 MCP 服务报 401:凭证解析优先取用当前有效凭证。 |
| 2026-07-22 | `v0.7.22-alpha.1347` | 外接 MCP 工具调用兼容性加固:工具注册护栏、参数数值精度保真、断连自动重连、schema 异常明确诊断。 |
| 2026-07-22 | `v0.7.22-alpha.0943` | 二进制文件预览改为十六进制转储(十六进制 + 字符对照);补齐 6 个文件类型图标并优化深色主题显示。 |
| 2026-07-21 | `v0.7.21-alpha.1945` | 文件预览支持语法高亮并按类型显示图标、可预览范围扩大到会话附加路径;新增国产化 Linux(deb)安装包(麒麟 V10 / V11 与 webkit 4.1)及独立升级通道。 |
| 2026-07-21 | `v0.7.21-alpha.1158` | 模型参数编辑入口收敛为唯一一处、按字段细粒度保存并加乐观锁;修复轨迹面板工具耗时始终显示 0。 |
| 2026-07-20 | `v0.7.20-alpha.2034` | 界面细节对齐:无状态条时右侧堆叠容器隐藏不占位,用量统计浮层右缘与上下文占用圆环精确对齐。 |
| 2026-07-20 | `v0.7.20-alpha.1453` | 宽屏下附加资料条改为右侧堆叠抽屉,快捷菜单新增「语言」滑块(简体 / 繁體 / English 直接切换);修复多工作区下工具网关管理接口返回 401。 |
