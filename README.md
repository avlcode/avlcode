...
| --- | --- | --- |
| 2026-07-17 | `v0.7.17-alpha.1623` | 工作区与会话新增「顶部标签页」布局(可与侧边栏切换、标签重启后恢复),快捷菜单新增「信息量」滑块,导出 HTML 与界面一致;修复 SSH 远程工作区自定义名称重启丢失、历史回放「在右侧打开」无响应。 |
| 2026-07-17 | `v0.7.17-alpha.1135` | 无限制模式下产生的消息以浅色边框整体圈出并明确标识,起止一目了然,子代理继承该模式的消息同样带标识。 |
| 2026-07-16 | `v0.7.16-alpha.1323` | 新增插件管理工具组(搜索 / 列出 / 安装 / 卸载,写入类操作两步确认);⋯ 菜单支持鼠标右键唤起并修正贴近底部被裁切。 |
| 2026-07-15 | `v0.7.15-alpha.1323` | 安全加固:以 stdio 启动的第三方 MCP 服务不再继承工具网关的管理密钥,消除越权风险。 |
| 2026-07-15 | `v0.7.15-alpha.1032` | 工具三态(启用 / 禁用 / 询问)真正生效并接入确认流程,增强工具调用错误自愈;修复插件导入的身份与目录问题。 |
